...
-too many connnections!!                                                            ---> BIG problem with intevallo
...
float TEMPr;
float TEMPrIN;
float webTemp = 15;          //float webTemp is the value on the temperature set from website - give an arbitrary value: 15
...
void loop() {
  ...
  currentTime = (millis() - protect);      //set current time and protect timer to the system runtime

  
  if(currentTime - ethernetMaintainTimeLapsed > 120000) {    //every 60 seconds maintain IP address
  ...
  if(internet == HIGH && currentTime > 6301){          //if the internet switch is set on internet and initial time has passed
    desiredT = webTemp;        //use webserver reading
    intervallo = 180000;        //keep website search interval to 3 min.
    lcd.setCursor(15,0);
    lcd.print("i");
  }else if(internet == LOW && currentTime > 6301){        //else, don't
    intervallo = 90000;      //set website search interval to 15 min.
    lcd.setCursor(15,0);
    lcd.print("l");
  }else if(currentTime <= 6000){
    lcd.setCursor(10,0);
    lcd.print("load..");
  }else if(currentTime <= 6301 && currentTime > 6000){
    lcd.setCursor(10,0);
    lcd.print("      ");
  }
 ...
 // reading2 = analogRead(sensor2Pin);      //*read sensors --- obsolete, replaced by for average loop to reduce fluctuation*
  ...
  if (currentTime > 7301){
   ...
   else if (currentTime >= 7000 && currentTime <= 7300){
     lcd.setCursor(7,1);
     lcd.print("         ");
   }
  ...
  if(currentTime < 8000 && currentTime > 17301){       // if desired lower than room then OFF
