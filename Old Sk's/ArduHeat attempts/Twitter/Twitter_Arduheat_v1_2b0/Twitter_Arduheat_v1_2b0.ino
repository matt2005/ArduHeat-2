...
unsigned int interval = 15000;  // delay between requests
...
void loop() {
  ...
  if(currentTime - lastAttemptTime > interval) {      //connect to twitter every 'interval' seconds
    connectToServer();
  ...
  if(internet == HIGH){          //if the internet switch is set on internet
           ...
           interval = 15000;        //keep twitter search interval to 15 seconds
           lcd.setCursor(15,0);
           lcd.print("i");
         }
         else if(internet == LOW){        //else, don't
           interval = 600000;      //set twitter search interval to 10 min.
           lcd.setCursor(15,0);
           lcd.print("l");
         }
