...
unsigned int intervallo = 15000;  // delay between requests
...
// attempt a DHCP connection:
 ...
 f = 20;      //initiate float f (tweet value) and give an arbitrary value: 20
}
...
void loop() {
  ...
  if(currentTime - ethernetMaintainTimeLapsed > 60000) {    //every 60 seconds maintain IP address
   // Ethernet.maintain();    //done next line down with return check
    byte returnedValue = Ethernet.maintain();
    lcd.setCursor(12,1);
    switch (returnedValue) {
      case 0: lcd.print(/*"nothing happened"*/ "0"); break;
      case 1: lcd.clear();
              lcd.print("renew failed");
              delay(10000);
              //Ethernet.begin(mac); break;                            //RESET CODE HERE FOR V.2.0 
              for(;;); break;
              lcd.clear();                  //then clear and prepare lcd for inputs from the loop()
              lcd.print("DESIRED T:");
              lcd.setCursor(0,1);
              ...
              lcd.setCursor(15, 0);
              lcd.print("~");
      case 2: lcd.print(/*"renew success"*/ "2"); break;
      case 3: lcd.print(/*"rebind fail"*/ "3"); break;
      case 4: lcd.print(/*"rebind success"*/ "4"); break;
  ...
  if(currentTime - lastAttemptTime > intervallo) {      //connect to twitter every 'interval' seconds
    connectToServer();
  ...
  //twitter reading bit
    ...
    if (client.available()) {
     ...
     // Serial.println(tweet);       *nope, pins 0 and 1 used*
        ...
        f = atof(carray+1);                  //convert the array into an float; f is the float containing the value of the tweet
  ...
  if(internet == HIGH){          //if the internet switch is set on internet
           desiredT = f;        //use twitter reading
           intervallo = 15000;        //keep twitter search interval to 15 seconds
           lcd.setCursor(15,0);
           lcd.print("i");
         }
         else if(internet == LOW){        //else, don't
           intervallo = 600000;      //set twitter search interval to 10 min.
           lcd.setCursor(15,0);
           lcd.print("l");
         }
...
else if(protect > 15000 && protect < 17500){
  lcd.setCursor(7,1);//set cursor to after the "current T" text
  lcd.print(TEMPr,1);//lcd print current temperature
  lcd.setCursor(11,1);
  lcd.print("  ");
