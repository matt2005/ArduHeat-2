...
-issues displaying a decent desired t when server returns error 406
-issues with ON/OFF code
...
void loop() {
  ...
  if(internet == HIGH && protect > 16301){          //if the internet switch is set on internet
    desiredT = f;        //use webserver reading
    intervallo = 180000;        //keep website search interval to 2 min.
    lcd.setCursor(15,0);
    lcd.print("i");
  }else if(internet == LOW && protect > 16301){        //else, don't
    intervallo = 600000;      //set website search interval to 10 min.
    lcd.setCursor(15,0);
    lcd.print("l");
 ...
 // reading2 = analogRead(sensor2Pin);      //*read sensors --- obsolete, replaced by for average loop to reduce fluctuation*
  ...
  if(protect > 17301){
    if(TEMPr - desiredT < -.5){       //desired - room > 0.5   =   if desired is more than 0.5 higher, then ON
      ...
      digitalWrite(0, LOW);
    }else if((TEMPr = desiredT) && protect > 20000){       // if desired lower than room then OFF
      digitalWrite(A5, LOW);
      lcd.setCursor(13,1);
      lcd.print("OFF");//heating status printed at end of screen
      digitalWrite(0, LOW);
    }else if((TEMPr = (desiredT - 0.5)) && protect > 20000){       // if desired lower than room then OFF
      digitalWrite(A5, LOW);
      lcd.setCursor(13,1);
      lcd.print("OFF");//heating status printed at end of screen
      digitalWrite(0, LOW);
...
int connectToServer() {
 ...
 //   Serial.println("disconnecting.");                                    SET LED TO MARK THIS STATUS ONE DAY
   /*     lcd.clear();
        lcd.print("error 406");
        lcd.setCursor(0,1);
        lcd.print
