...
-issues with bouncing room t when changing ON/OFF
...
float TEMPr;
float TEMPrIN;
float webTemp;          //float webTemp is the value on the temperature set from website
...
unsigned int intervallo = 120000;  // delay between requests
...
// attempt a DHCP connection:
 ...
 webTemp = 20;      //initiate float f (webtemp value) and give an arbitrary value: 20
}
...
void loop() {
 ...
 // reading2 = analogRead(sensor2Pin);      //*read sensors --- obsolete, replaced by for average loop to reduce fluctuation*
  ...
  if(protect < 19000){       // if desired lower than room then OFF
