...
-tick *scroll temp* tick                                                            ---> SOLVED --->initiate protect after connect to server mannaggia!
...
void loop() {
  ...
  if(currentTime - ethernetMaintainTimeLapsed > 120000) {    //every 120 seconds maintain IP address
 ...
 // reading2 = analogRead(sensor2Pin);      //*read sensors --- obsolete, replaced by for average loop to reduce fluctuation*
  ...
  if(currentTime < 8000 && currentTime > 7301){       // if desired lower than room then OFF
